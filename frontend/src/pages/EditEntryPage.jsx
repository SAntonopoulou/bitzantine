...
export default function EditEntryPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Failed to update entry: ${errorData.detail || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto text-stone-200">
      ...
      <h1 className="text-3xl font-bold mb-8 text-amber-500">Edit Entry: {entry.title}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-stone-800 p-8 rounded-lg shadow-lg">
        <div>
          ...
          <input 
            ...
            value={entry.title} onChange={handleInputChange}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select 
              name="entry_type"
              value={entry.entry_type} onChange={handleInputChange}
              className="mt-1 block w-full rounded-md bg-stone-700 border-stone-600 shadow-sm"
            >
          ...
          <div>
            ...
            <select 
              name="era_id"
              value={entry.era_id || ''} onChange={handleInputChange}
              required={entry.entry_type === 'evolving'}
              className="mt-1 block w-full rounded-md bg-stone-700 border-stone-600 shadow-sm"
            >
        ...
        <div>
          ...
          <input 
            type="text" name="tags"
            value={entry.tags} onChange={handleInputChange}
            className="mt-1 block w-full rounded-md bg-stone-700 border-stone-600 shadow-sm"
          />
        </div>
