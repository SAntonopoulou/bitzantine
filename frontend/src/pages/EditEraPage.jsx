...
export default function EditEraPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Construct the update payload with only the editable fields
    const eraUpdateData = {
      name: era.name,
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Failed to update era: ${errorData.detail || 'Unknown error'}`);
      }
    } catch (error) {
      alert("Error updating era. Check console for details.");
    }
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto text-stone-200">
      ...
      <h1 className="text-3xl font-bold mb-8 text-amber-500">Edit Era: {era.name}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-stone-800 p-8 rounded-lg shadow-lg">
        <div>
          ...
          <input 
            ...
            value={era.name} onChange={handleInputChange}
            className="mt-1 block w-full rounded-md bg-stone-700 border-stone-600 shadow-sm"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <input 
              type="color" name="color_hex"
              value={era.color_hex} onChange={handleInputChange}
          ...
          <div>
            ...
            <input 
              ...
              value={era.start_date} onChange={handleInputChange}
              className="mt-1 block w-full rounded-md bg-stone-700 border-stone-600 shadow-sm"
            />
          </div>
          <div>
            ...
            <input 
              ...
              value={era.end_date || ''} onChange={handleInputChange}
              disabled={era.is_current_era}
              className="mt-1 block w-full rounded-md bg-stone-700 border-stone-600 shadow-sm disabled:bg-stone-600"
            />
          </div>
        </div>
        ...
        <div className="flex items-center">
          <input 
            type="checkbox" id="isCurrent" name="is_current_era"
            checked={era.is_current_era} onChange={handleInputChange}
