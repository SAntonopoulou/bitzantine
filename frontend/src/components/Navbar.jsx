...
export default function Navbar() {
  ...
  const MobileNav = () => (
    <>
      <div className={`fixed top-0 left-0 h-full w-full bg-black/50 z-50 transition-opacity duration-300 md:hidden ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={toggleMenu}></div>
      <div className={`fixed top-0 left-0 h-full w-64 bg-stone-800 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out md:hidden flex flex-col ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-stone-700 flex-shrink-0"><SiteLogo isMobile={true} /></div>
          ...
          <div className="flex flex-col space-y-2 p-4">
            ...
            {user ? (
              <>
                ...
                {['admin', 'super_admin'].includes(user.role) && <Link to="/admin" className={mobileLinkStyles} onClick={toggleMenu}>Admin</Link>}
      ...
      {user ? (
        <div className="relative" ref={profileMenuRef}>
          <button onClick={toggleProfileMenu} className="flex items-center gap-2 hover:bg-stone-700 p-2 rounded-md transition-colors">
            <img src={user.avatar_url ? `${API_URL}${user.avatar_url}` : `https://ui-avatars.com/api/?name=${user.display_name || user.username}&background=292524&color=f59e0b`} alt="avatar" className="w-8 h-8 rounded-full object-cover" />
          ...
          {isProfileOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-stone-800 border border-stone-700 rounded-md shadow-lg z-20 py-1">
              ...
              {['admin', 'super_admin'].includes(user.role) && (<><div className="my-1 h-px bg-stone-700"></div><Link to="/admin" className="block px-4 py-2 text-sm text-amber-400 hover:bg-stone-700" onClick={toggleProfileMenu}>Admin</Link></>)}
